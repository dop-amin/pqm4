...
void InnerProd(uint16_t pkcl[SABER_K][SABER_N],uint16_t skpv[SABER_K][SABER_N],uint16_t res[SABER_N]);
void MatrixVectorMul(polyvec *a, uint16_t skpv[SABER_K][SABER_N], uint16_t res[SABER_K][SABER_N], int16_t transpose);

void MatrixVectorMul_keypair(const unsigned char *seed, uint16_t skpv[SABER_K][SABER_N], uint16_t res[SABER_K][SABER_N]);
void MatrixVectorMul_encryption(const unsigned char *seed, uint16_t skpv[SABER_K][SABER_N], unsigned char *ciphertext);
...
static void byte_bank2pol_part(unsigned char *bytes, uint16_t pol_part[], uint16_t pol_part_start_index, uint16_t num_8coeff)
{
    uint32_t j;
...
static void GenMatrix_poly(uint16_t temp_ar[], const unsigned char *seed, uint16_t poly_number)
{
#if Saber_type == 3
  ...
  static uint16_t poly_complete;
    ...
    if(poly_number==0)
    {
        shake128_absorb(&s, seed, SABER_SEEDBYTES);
  ...
  while(poly_complete!=1)
  {
    ...
    shake128_squeezeblocks(shake_op_buf+left_over_bytes, 1, &s);
...
void indcpa_kem_keypair(unsigned char *pk, unsigned char *sk)
{
  ...
  //------------------------do the matrix vector multiplication and rounding------------
    ...
    MatrixVectorMul_keypair(&pk[SABER_POLYVECCOMPRESSEDBYTES],skpv,res);
...
void indcpa_kem_enc(unsigned char *message_received, unsigned char *noiseseed, const unsigned char *pk, unsigned char *ciphertext)
{
    ...
    MatrixVectorMul_encryption(&pk[SABER_POLYVECCOMPRESSEDBYTES], skpv1, ciphertext);
...
static unsigned char MatrixVectorMul_encryption_cmp(const unsigned char *seed, uint16_t skpv[SABER_K][SABER_N], unsigned char *ciphertext)
{
...
unsigned char indcpa_kem_enc_cmp(unsigned char *message_received, unsigned char *noiseseed, const unsigned char *pk, unsigned char *ciphertext)
{
    ...
    fail |= MatrixVectorMul_encryption_cmp(&pk[SABER_POLYVECCOMPRESSEDBYTES], skpv1, ciphertext);
...
//********************client matrix-vector multiplication ends************************************
    ...
    uint32_t fail32 = (uint32_t)(~fail) + 1;
    fail32 >>= 8;

    return (unsigned char)fail32;
}
